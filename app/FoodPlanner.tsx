...
const foods: Record<string, Food> = {
  ...
  "Fette biscottate integrali": { kcal: 397, protein: 11, carbs: 72, fat: 7.5, fiber: 7, source: "CREA" },
  "Confettura di frutta": { kcal: 250, protein: 0.4, carbs: 62, fat: 0.1, fiber: 1.2, source: "CREA" },
  "Latte parzialmente scremato": { kcal: 46, protein: 3.3, carbs: 4.9, fat: 1.6, fiber: 0, source: "CREA" },
  "Bevanda di soia senza zucchero": { kcal: 33, protein: 2.9, carbs: 1.7, fat: 1.8, fiber: 0.6, source: "USDA" },
  "Bevanda d'avena senza zucchero": { kcal: 43, protein: 1, carbs: 7, fat: 1.5, fiber: 0.8, source: "USDA" },
  "Succo d'arancia 100%": { kcal: 45, protein: 0.7, carbs: 10.4, fat: 0.2, fiber: 0.2, source: "USDA" },
  "Biscotti secchi": { kcal: 430, protein: 7.5, carbs: 75, fat: 11, fiber: 3, source: "CREA" },
  "Cracker integrali": { kcal: 421, protein: 10, carbs: 68, fat: 13, fiber: 7, source: "USDA" },
  "Burro": { kcal: 717, protein: 0.9, carbs: 0.1, fat: 81.1, fiber: 0, source: "CREA" },
};
...
const recipes: Recipe[] = [
  ...
  { id:"eggs-quinoa", name:"Uova, quinoa e spinaci cremosi", kicker:"Colazione salata o pranzo leggero", image:photo("eggs-quinoa-v3"), time:14,
...
const generatedRecipes: Recipe[] = Array.from({length:284},(_,index)=>{
  ...
  const image=base[0]==="Noodles di riso cotti"?photo("rice-noodles-veg-v3"):course==="Contorno"&&veg[2]==="zucchine e pomodorini"?photo("zucchini-tomato-side-v3"):proteinKey.includes("salmone")?photo("salmon"):proteinKey.includes("tonno")?photo("tuna-rice"):proteinKey.includes("pollo")?photo("chicken-bowl"):protein[0]==="Uovo"?photo("eggs-quinoa-v3"):photo("farro");
...
const snackRecipes:Recipe[]=Array.from({length:32},(_,index)=>{const fruit=snackFruits[Math.floor(index/4)%snackFruits.length];const nuts=snackNuts[Math.floor(index/8)%snackNuts.length];const kind=["Colazione","Spuntino","Dolce","Gelato"][index%4];const frozen=kind==="Gelato";return {id:`dolce-${index+1}`,name:frozen?`Frozen yogurt a ${fruit.toLowerCase()} e ${nuts.toLowerCase()}`:`${kind} · coppa di ${fruit.toLowerCase()}, yogurt e ${nuts.toLowerCase()}`,kicker:`${kind} ${kind==="Colazione"?"bilanciata":"bilanciato"} con quantità modificabili`,course:kind,cuisine:"Gourmet",image:photo("fruit-breakfast-v2"),time:frozen?185:8,ingredients:[{food:"Yogurt greco 2%",grams:180},{food:fruit,grams:150},{food:nuts,grams:12},{food:index%3===0?"Cioccolato fondente 70%":"Fiocchi d'avena",grams:index%3===0?10:20}],steps:frozen?[`Lava e taglia ${fruit.toLowerCase()} in pezzi piccoli; tienine alcuni da parte per la finitura.`,`Frulla yogurt e frutta per 30-40 secondi, senza aggiungere zucchero. Versa in un contenitore basso.`,`Congela per 3 ore, mescolando energicamente ogni 45 minuti per rompere i cristalli. Prima di servire lascialo 8 minuti a temperatura ambiente.`,`Completa con ${nuts.toLowerCase()} tritati e la finitura prevista; usa una coppa piccola per mantenere la porzione chiara.`]:[`Lava e taglia ${fruit.toLowerCase()} in pezzi regolari.`,`Tosta avena o frutta secca per 2 minuti in padella asciutta, mescolando e senza farla scurire troppo.`,`Metti lo yogurt nella coppa, aggiungi la frutta e completa con ${nuts.toLowerCase()} tritati.`,`Servi subito se vuoi contrasto croccante; lascia 10 minuti se preferisci l'avena più morbida.`],alternatives:["Cambia frutto mantenendo una quantità simile","Usa un'alternativa vegetale compatibile con i tuoi filtri","Aumenta o riduci i grammi e registra la quantità reale"]}});
const simpleBreakfasts:Recipe[]=[
  {id:"breakfast-rusks-jam",name:"Fette biscottate, ricotta e confettura",kicker:"Colazione pronta in tre minuti",course:"Colazione",cuisine:"Italiano",image:photo("rusks-ricotta-jam-v3"),time:3,ingredients:[{food:"Fette biscottate integrali",grams:45},{food:"Ricotta vaccina",grams:80},{food:"Confettura di frutta",grams:25},{food:"Arancia",grams:150}],steps:["Pesa fette biscottate, ricotta e confettura.","Spalma prima la ricotta e poi un velo di confettura; servi con l'arancia già lavata e tagliata."],alternatives:["Yogurt al posto della ricotta","Miele al posto della confettura"]},
  {id:"breakfast-milk-biscuits",name:"Latte, biscotti secchi e frutta",kicker:"La colazione più immediata",course:"Colazione",cuisine:"Italiano",image:photo("fruit-breakfast-v2"),time:2,ingredients:[{food:"Latte parzialmente scremato",grams:250},{food:"Biscotti secchi",grams:45},{food:"Mela",grams:150}],steps:["Versa il latte nella tazza e scaldalo 60-90 secondi se lo preferisci caldo.","Pesa i biscotti e mangia la mela intera o tagliata a spicchi."],alternatives:["Pera al posto della mela","Yogurt al posto del latte"]},
  {id:"breakfast-crackers-ricotta",name:"Cracker integrali, ricotta e miele",kicker:"Salata-dolce, pronta senza cucinare",course:"Colazione",cuisine:"Italiano",image:photo("rusks-ricotta-jam-v3"),time:3,ingredients:[{food:"Cracker integrali",grams:50},{food:"Ricotta vaccina",grams:90},{food:"Miele",grams:10},{food:"Kiwi",grams:130}],steps:["Pesa gli ingredienti e pela il kiwi.","Spalma la ricotta sui cracker, aggiungi il miele a filo e mangia il kiwi a parte."],alternatives:["Confettura al posto del miele","Fette biscottate al posto dei cracker"]},
  {id:"breakfast-rusks-butter",name:"Fette biscottate, burro e confettura",kicker:"Classica, con quantità chiare",course:"Colazione",cuisine:"Italiano",image:photo("rusks-ricotta-jam-v3"),time:3,ingredients:[{food:"Fette biscottate integrali",grams:45},{food:"Burro",grams:10},{food:"Confettura di frutta",grams:25},{food:"Latte parzialmente scremato",grams:200}],steps:["Lascia ammorbidire il burro un minuto e pesane la quantità.","Spalmalo sulle fette biscottate, aggiungi la confettura e accompagna con il latte."],alternatives:["Ricotta al posto del burro","Yogurt al posto del latte"]}
];
const allRecipes=[...simpleBreakfasts,...recipes,...generatedRecipes,...snackRecipes];
...
export function FoodPlanner(){
  ...
  const activityDelta=check.todayActivity==="intensa"?200:check.todayActivity==="leggera"?75:-75;
  const plannedCalories=Math.max(1200,calories+activityDelta);
  const plannedDrinkMap:Record<string,RecipeIngredient[]>={"Acqua":[],"Latte parz. scremato":[{food:"Latte parzialmente scremato",grams:250}],"Bevanda di soia":[{food:"Bevanda di soia senza zucchero",grams:250}],"Bevanda d'avena":[{food:"Bevanda d'avena senza zucchero",grams:250}],"Spremuta d'arancia":[{food:"Succo d'arancia 100%",grams:250}],"Frullato banana e soia":[{food:"Bevanda di soia senza zucchero",grams:200},{food:"Banana",grams:100}],"Frullato frutti e avena":[{food:"Bevanda d'avena senza zucchero",grams:200},{food:"Frutti di bosco",grams:100}],"Coca-Cola Zero":[],"Gassata zero":[]};
  const plannedDrinkMacros=calc(plannedDrinkMap[plannedDrink]||[]);
  const targetForDay=(day:number)=>day===dayIndex?plannedCalories:calories;
  const recipeCuisine=(r:Recipe)=>r.cuisine||(r.id.includes("toast")||r.id.includes("sweet")?"Italiano":"Mediterraneo");
  const recipeCourse=(r:Recipe)=>r.course||(["jar","toast","apple-oats","toast-tuna","eggs-quinoa"].includes(r.id)?"Colazione":r.id.startsWith("snack-")?"Spuntino":"Piatto unico");
  const compatibleWithSlot=(r:Recipe)=>{if(!swapTarget)return true;const course=recipeCourse(r);return swapTarget.slot===0?course==="Colazione":swapTarget.slot===2?["Spuntino","Dolce","Gelato"].includes(course):["Piatto unico","Primo","Secondo"].includes(course)};
  const filteredRecipes=allRecipes.filter(r=>isAllowed(r)&&compatibleWithSlot(r)&&(cuisineFilter==="Tutte"||recipeCuisine(r)===cuisineFilter)&&r.name.toLowerCase().includes(libraryQuery.toLowerCase()));
  const baseTotal=useMemo(()=>currentIds.reduce((s,id)=>s+calc(recipeMap[id].ingredients).kcal,0),[dayIndex,choices]);
  const scale=Math.max(1000,plannedCalories-plannedDrinkMacros.kcal)/baseTotal;
  const dayTotals=useMemo(()=>currentIds.reduce((s,id)=>{const m=calc(recipeMap[id].ingredients,scale); return {kcal:s.kcal+m.kcal,protein:s.protein+m.protein,carbs:s.carbs+m.carbs,fat:s.fat+m.fat};},{kcal:plannedDrinkMacros.kcal,protein:plannedDrinkMacros.protein,carbs:plannedDrinkMacros.carbs,fat:plannedDrinkMacros.fat}),[dayIndex,scale,choices,plannedDrink]);
  ...
  const chooseRecipe=(recipe:Recipe)=>{if(swapTarget){setChoices(v=>({...v,[`${swapTarget.day}-${swapTarget.slot}`]:recipe.id}));setDayIndex(swapTarget.day);setSwapTarget(null);setTab(swapReturnTab);scrollTo({top:0,behavior:"smooth"});}else{setSelectedMealKey(null);setSelected(recipe)}};
  ...
  const dayScale=(day:number)=>{const ids=getDayIds(day);const drinkKcal=day===dayIndex?plannedDrinkMacros.kcal:0;return Math.max(1000,targetForDay(day)-drinkKcal)/ids.reduce((s,id)=>s+calc(recipeMap[id].ingredients).kcal,0)};
  ...
  const replanNextDay=(day:number)=>{if(day>=days.length-1){setReplanNote("Settimana completata: usa il diario per impostare la prossima.");return}const total=loggedTotal(day);const target=targetForDay(day);const safe=allRecipes.filter(r=>isAllowed(r)&&!["Contorno","Dolce","Gelato"].includes(r.course||""));const ranked=[...safe].sort((a,b)=>calc(a.ingredients).kcal-calc(b.ingredients).kcal);const pool=total>target*1.1?ranked.slice(0,Math.max(2,Math.floor(ranked.length/2))):total<target*.8?ranked.slice(Math.floor(ranked.length/2)):safe;setChoices(v=>({...v,[`${day+1}-1`]:pool[(day*2)%pool.length].id,[`${day+1}-3`]:pool[(day*2+1)%pool.length].id}));setReplanNote(total>target*1.1?"Domani: pasti regolari e sazianti, senza saltare nulla.":total<target*.8?"Domani: pasti completi e spuntino confermato.":"Domani aggiornato mantenendo l'equilibrio.")};
  const rebalanceRemaining=(day:number)=>{const open=[0,1,2,3].filter(slot=>!completed[`${day}-${slot}`]);if(!open.length){setReplanNote("Tutti i pasti di oggi sono già registrati.");return}const remaining=Math.max(0,targetForDay(day)-loggedTotal(day));const average=remaining/open.length;const safe=allRecipes.filter(isAllowed);const updates:Record<string,string>={};open.forEach((slot,n)=>{const coursePool=slot===2?safe.filter(r=>["Spuntino","Dolce","Gelato"].includes(r.course||"")):safe.filter(r=>!["Spuntino","Dolce","Gelato","Contorno"].includes(r.course||""));const ranked=[...(coursePool.length?coursePool:safe)].sort((a,b)=>Math.abs(calc(a.ingredients).kcal-average)-Math.abs(calc(b.ingredients).kcal-average));updates[`${day}-${slot}`]=ranked[n%Math.min(ranked.length,12)].id});setChoices(v=>({...v,...updates}));setReplanNote(`Restano circa ${round(remaining)} kcal: ho aggiornato ${open.length} proposte.`)};
  const shoppingItems=useMemo(()=>{const totals:Record<string,number>={};const targetDays=shoppingScope==="day"?[dayIndex]:days.map((_,i)=>i);targetDays.forEach(day=>getDayIds(day).forEach(id=>recipeMap[id].ingredients.forEach(x=>{totals[x.food]=(totals[x.food]||0)+x.grams*dayScale(day)})));if(targetDays.includes(dayIndex))(plannedDrinkMap[plannedDrink]||[]).forEach(x=>{totals[x.food]=(totals[x.food]||0)+x.grams});return Object.entries(totals).map(([food,grams])=>({food,grams:round(grams)})).sort((a,b)=>a.food.localeCompare(b.food));},[shoppingScope,dayIndex,choices,calories,plannedDrink]);
  const shareShopping=async()=>{const text=`Lista spesa ${shoppingScope==="day"?days[dayIndex].label:"settimanale"}\n`+shoppingItems.filter(x=>!groceryChecked[x.food]).map(x=>`• ${x.food}: ${groceryAmounts[x.food]??x.grams} g`).join("\n");if(navigator.share)await navigator.share({title:"Lista spesa - Tavola Mia",text});else await navigator.clipboard.writeText(text)};
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand-mark">T</div><div><strong>Tavola Mia</strong><span>menu, ricette e diario</span></div><span className="version">v{VERSION}</span></header>
    <section className="content">
      {tab==="today" && <>
        <section className="today-strip"><div><span>OGGI · {days[dayIndex].label}</span><b>{round(dayTotals.kcal)} kcal · {activityDelta>0?`+${activityDelta}`:activityDelta} attività</b></div><button onClick={()=>setPreferencesOpen(v=>!v)}>⚙ Filtri {blockedFoods.length?`(${blockedFoods.length})`:""}</button></section>
        ...
        <section className="compact-config"><div><label>Obiettivo</label><select value={goal} onChange={e=>setGoal(e.target.value)}><option>Dimagrimento graduale</option><option>Equilibrio</option><option>Mantenimento massa</option></select></div><div><label>Base kcal</label><select value={calories} onChange={e=>setCalories(Number(e.target.value))}>{[1400,1600,1800,2000,2200,2400].map(x=><option key={x}>{x}</option>)}</select></div></section>
        <section className="cuisine-picker"><div><span>Stile</span><select value={cuisineChoice} onChange={e=>setCuisineChoice(e.target.value)}>{["Italiano","Asiatico","Gourmet","Mediterraneo","Vegetale"].map(x=><option key={x}>{x}</option>)}</select></div><div><span>Bevanda</span><select value={plannedDrink} onChange={e=>setPlannedDrink(e.target.value)}>{["Acqua","Latte parz. scremato","Bevanda di soia","Bevanda d'avena","Spremuta d'arancia","Frullato banana e soia","Frullato frutti e avena","Coca-Cola Zero","Gassata zero"].map(x=><option key={x}>{x}</option>)}</select></div><button onClick={applyCuisine}>Proponi pranzo e cena</button></section>
        <button className="shopping-trigger" onClick={()=>{setShoppingScope("day");setShoppingOpen(true)}}>🛒 Lista della spesa di oggi</button>
        <section className="checkin compact"><button className="checkin-toggle" onClick={()=>setCheckinOpen(v=>!v)}><span>Check-in</span><b>{check.todayActivity==="no"?"Riposo":check.todayActivity} · {check.feeling}</b><i>{checkinOpen?"−":"＋"}</i></button>{checkinOpen&&<div className="checkin-body"><div className="question"><span>Ieri</span><div className="chips">{[["regolare","Regolare"],["poco","Meno"],["molto","Più"]].map(([v,l])=><button className={check.yesterday===v?"active":""} key={v} onClick={()=>setCheck({...check,yesterday:v})}>{l}</button>)}</div></div><div className="question"><span>Attività oggi</span><div className="chips">{[["no","Riposo"],["leggera","Leggera"],["intensa","Intensa"]].map(([v,l])=><button className={check.todayActivity===v?"active":""} key={v} onClick={()=>setCheck({...check,todayActivity:v})}>{l}</button>)}</div></div><div className="question"><span>Stato</span><div className="chips">{[["bene","Bene"],["gonfio","Gonfio"],["stanco","Stanco"],["fame","Fame"]].map(([v,l])=><button className={check.feeling===v?"active":""} key={v} onClick={()=>setCheck({...check,feeling:v})}>{l}</button>)}</div></div><div className="question"><span>Domani</span><div className="chips">{[["no","Riposo"],["leggera","Leggera"],["intensa","Intensa"]].map(([v,l])=><button className={check.tomorrowActivity===v?"active":""} key={v} onClick={()=>setCheck({...check,tomorrowActivity:v})}>{l}</button>)}</div></div></div>}</section>
        ...
        {getDayIds(dayIndex).some((_,slot)=>completed[`${dayIndex}-${slot}`])&&<button className="primary-btn compact" onClick={()=>rebalanceRemaining(dayIndex)}>Riequilibra ciò che resta oggi</button>}
        ...
        <section><div className="section-title"><div><span className="eyebrow">IL TUO MENU</span><h2>Quattro momenti, zero dubbi</h2></div><button className="text-btn" onClick={()=>setTab("week")}>7 giorni</button></div>
          <div className="meal-list">{currentIds.map((id,i)=>{const r=recipeMap[id];const m=calc(r.ingredients,scale);const key=`${dayIndex}-${i}`;const allowed=isAllowed(r);const actual=completed[key]?calc(actualIngredients(key,r)):null;return <article className={`meal-card ${allowed?"":"blocked"}`} key={key} onClick={()=>{setSelectedMealKey(key);setSelected(r)}}><img src={r.image} alt={r.name}/><div className="meal-body"><span>{["Colazione","Pranzo","Spuntino","Cena"][i]}</span><h3>{r.name}</h3><p>{allowed?actual?`Mangiato · ${round(actual.kcal)} kcal`:`${round(m.kcal)} kcal · ${round(m.protein)}g · ${r.time} min`:"Contiene un alimento escluso"}</p></div><div className="meal-actions"><button aria-label={`Cambia ${["colazione","pranzo","spuntino","cena"][i]}`} onClick={e=>{e.stopPropagation();setSwapReturnTab("today");setSwapTarget({day:dayIndex,slot:i});setTab("library")}}>↻</button><button aria-label={completed[key]?"Segna da fare":"Segna mangiato"} className={`check ${completed[key]?"done":""}`} onClick={e=>{e.stopPropagation();setSelectedMealKey(key);setSelected(r)}}>{completed[key]?"✓":""}</button></div></article>})}</div>
        </section>
      </>}
      {tab==="week" && <section><span className="eyebrow">SETTIMANA</span><h1 className="page-title">Menu e andamento</h1><button className="shopping-trigger" onClick={()=>{setShoppingScope("week");setShoppingOpen(true)}}>🛒 Lista della spesa settimanale</button><div className="week-plan">{days.map((d,i)=><article key={d.label} className="week-day"><header><div><span>{d.label}</span><b>{d.mood}</b></div><button onClick={()=>{setDayIndex(i);setTab("today");scrollTo({top:0,behavior:"smooth"})}}>Apri</button></header>{getDayIds(i).map((id,slot)=>{const r=recipeMap[id];const key=`${i}-${slot}`;return <div className="week-meal-row" key={key}><button className="week-meal" onClick={()=>{setSelectedMealKey(key);setSelected(r)}}><img src={r.image} alt=""/><span><small>{["Colazione","Pranzo","Spuntino","Cena"][slot]}</small><b>{r.name}</b></span><i>{completed[key]?"✓":"→"}</i></button><button className="week-change" aria-label={`Cambia ${["colazione","pranzo","spuntino","cena"][slot]} ${d.label}`} onClick={()=>{setSwapReturnTab("week");setSwapTarget({day:i,slot});setTab("library")}}>↻</button></div>})}</article>)}</div></section>}
      ...
      {tab==="progress"&&(()=>{const total=loggedTotal(diaryDay);const target=targetForDay(diaryDay);const difference=round(total-target);const completedMeals=getDayIds(diaryDay).map((id,slot)=>({recipe:recipeMap[id],slot,key:`${diaryDay}-${slot}`})).filter(x=>completed[x.key]);return <section><span className="eyebrow">DIARIO</span><h1 className="page-title">{days[diaryDay].label}</h1><div className="diary-tabs">{days.map((d,i)=><button className={diaryDay===i?"active":""} key={d.label} onClick={()=>setDiaryDay(i)}>{i+1}</button>)}</div><div className="diary-total"><div><span>Registrato</span><strong>{round(total)} <small>kcal</small></strong></div><div><span>Target</span><b>{target} kcal</b><em className={Math.abs(difference)<=100?"ok":difference>0?"over":"under"}>{difference>0?"+":""}{difference} kcal</em></div></div><h2 className="mini-title">Cosa hai mangiato</h2><div className="diary-meals">{completedMeals.length?completedMeals.map(({recipe,key,slot})=><button key={key} onClick={()=>{setDayIndex(diaryDay);setSelectedMealKey(key);setSelected(recipe)}}><span>{["Colazione","Pranzo","Spuntino","Cena"][slot]}</span><b>{recipe.name}</b><i>{round(calc(actualIngredients(key,recipe)).kcal)} kcal</i></button>):<p>Nessun pasto registrato.</p>}</div><h2 className="mini-title">Bevande</h2><div className="quick-log">{drinkOptions.map(d=><button key={d.label} onClick={()=>addDrink(diaryDay,d)}>＋ {d.label}</button>)}</div><div className="log-list">{(drinks[diaryDay]||[]).map((x,i)=><button key={`${x.label}-${i}`} onClick={()=>setDrinks(v=>({...v,[diaryDay]:v[diaryDay].filter((_,n)=>n!==i)}))}><span>{x.label} · {x.amount}</span><b>{x.kcal} kcal ×</b></button>)}</div><h2 className="mini-title">Extra o sgarro</h2><div className="extra-form"><input placeholder="Cosa hai mangiato?" value={extraName} onChange={e=>setExtraName(e.target.value)}/><input aria-label="Calorie extra" type="number" placeholder="kcal" value={extraKcal} onChange={e=>setExtraKcal(e.target.value)}/><button onClick={addExtra}>Aggiungi</button></div><div className="log-list">{(extras[diaryDay]||[]).map((x,i)=><button key={`${x.label}-${i}`} onClick={()=>setExtras(v=>({...v,[diaryDay]:v[diaryDay].filter((_,n)=>n!==i)}))}><span>{x.label}</span><b>{x.kcal} kcal ×</b></button>)}</div><div className="diary-actions"><button onClick={()=>rebalanceRemaining(diaryDay)}>Riequilibra il resto</button><button onClick={()=>replanNextDay(diaryDay)}>Prepara domani</button></div>{replanNote&&<div className="replan-note">{replanNote}</div>}<div className="history">{days.map((d,i)=>{const n=[0,1,2,3].filter(slot=>completed[`${i}-${slot}`]).length;const dayLogged=loggedTotal(i);return <div key={d.label}><span>{d.label}</span><div><i style={{width:`${n/4*100}%`}}/></div><b>{dayLogged?`${round(dayLogged)}k`:`${n}/4`}</b></div>})}</div><div className="source-card"><b>Nessuna punizione</b><p>Una giornata diversa non richiede digiuno: registra, osserva e torna a pasti regolari.</p></div><div className="links"><a href="https://www.crea.gov.it/-/on-line-le-linee-guida-per-una-sana-alimentazione-2018" target="_blank">CREA ↗</a><a href="https://www.efsa.europa.eu/en/topics/topic/dietary-reference-values" target="_blank">EFSA ↗</a><a href="https://fdc.nal.usda.gov/about-us/" target="_blank">USDA ↗</a></div></section>})()}
    </section>
